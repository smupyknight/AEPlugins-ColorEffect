	...
	PF_LayerDef		*output)
{
	...
	PF_ADD_FLOAT_SLIDERX("Noise Size", 
						FILTER_NOISE_MIN,
						FILTER_NOISE_MAX,
						SLIDER_MIN,
						SLIDER_MAX,
	...
	PF_ADD_POPUP(
		...
		13,
		5,
		"COPY | BEHIND | IN FRONT | DISSOLVE | ADD | MULTIPLY | SCREEN | OVERLAY | SOFT LIGHT | HARD LIGHT | DARKEN | LIGHTEN | DIFFERENCE",
		BLEND_TYPE_ID);
	...
	PF_LayerDef		*output )
{
	...
	if (in_data->appl_id == 'PrMr') {
		...
		worldTransformSuite->copy(	in_data->effect_ref,			// This effect ref (unique id)
	...
	// overlay
	// create new transform and scale it.
	PF_EffectWorld colorWorld;/*
	PF_FloatMatrix *m_transFormMatrix;
	CCU_SetIdentityMatrix(m_transFormMatrix);*/

	int widthL = output->width;
	int heightL = output->height;
	PF_NewWorldFlags	flags = PF_NewWorldFlag_CLEAR_PIXELS;
	PF_Pixel overlayColor;
	overlayColor.alpha = (int)(params[OVERLAY_OPACITY]->u.sd.value) * 255 / 100;
	overlayColor.blue = params[OVERLAY_COLOR]->u.cd.value.blue;
	overlayColor.green = params[OVERLAY_COLOR]->u.cd.value.green;
	overlayColor.red = params[OVERLAY_COLOR]->u.cd.value.red;

	AEFX_CLR_STRUCT(colorWorld);
	...
	mode.xfer = params[BLEND_TYPE]->u.pd.value;
	mode.rand_seed = 0;
	...
	m_compRect.top = 0;
	m_compRect.left = 0;
	m_compRect.right = widthL;
	m_compRect.bottom = heightL;
